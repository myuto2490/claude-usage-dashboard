...
def _log(msg=""):
    ...
    AI_USAGE_LOGFILE にパスを指定すると、起動の各段階をファイルにも
    追記する (exe のトラブルシュート用)。
    """
    logfile = os.environ.get("AI_USAGE_LOGFILE")
    if logfile:
        try:
...
# ---------------------------------------------------------------------------
# 価格表 (per 1M tokens, USD)
#
# 公開単価は変更されることがあるため、機械可読な公開単価表 (LiteLLM の
# model_prices_and_context_window.json) を1日1回取得して使う。取得できない
# 間は下の内蔵表で概算する (取得失敗時は一定間隔で再試行)。
# 単価の形式: {"in","out","cr","cw5","cw1"} すべて USD / 1M tokens。
# ---------------------------------------------------------------------------
PRICING_URL = os.environ.get(
    "AI_USAGE_PRICING_URL",
    "https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json")
PRICING_CACHE = os.path.join(HOME, ".ai_usage_dashboard", "pricing.json")
PRICING_RETRY = int(os.environ.get("AI_USAGE_PRICING_RETRY", "3600"))  # 失敗時の再試行間隔(秒)
_pricing_lock = threading.Lock()
_pricing_state = {"data": None, "retry_at": 0.0}


def _builtin_price(model: str):
    """内蔵の概算単価 (公開単価表が取得できないときのフォールバック)。"""
    m = (model or "").lower()
    if "fable" in m or "mythos" in m:
        pin, pout = 10.0, 50.0
    elif "opus" in m:
        # Opus 4.5〜4.8 は 5/25。旧 Opus(4.1/4.0/3) は 15/75。
        pin, pout = (5.0, 25.0) if any(v in m for v in ("4-8", "4-7", "4-6", "4-5")) else (15.0, 75.0)
    elif "sonnet" in m:
        pin, pout = 3.0, 15.0
    elif "haiku" in m:
        pin, pout = (1.0, 5.0) if "4-5" in m else (0.80, 4.0)
    else:
        pin, pout = 3.0, 15.0   # 不明なモデルは Sonnet 相当で概算
    # 公式の倍率: cache_write 5分=1.25x / 1時間=2.0x, cache_read=0.1x
    return {"in": pin, "out": pout, "cr": pin * 0.10, "cw5": pin * 1.25, "cw1": pin * 2.0}


def _fetch_public_pricing():
    """公開単価表から Anthropic モデルの単価を抽出して返す。"""
    req = urllib.request.Request(
        PRICING_URL,
        headers={"Accept": "application/json", "User-Agent": "claude-usage-dashboard"})
    with urllib.request.urlopen(req, timeout=20) as r:
        raw = json.loads(r.read().decode("utf-8", "ignore"))
    models = {}
    for key, d in raw.items():
        if not isinstance(d, dict) or d.get("litellm_provider") != "anthropic":
            continue
        pin = d.get("input_cost_per_token")
        pout = d.get("output_cost_per_token")
        ...
        models[key.lower()] = {
            "in": pin,
            "out": pout,
            "cr": (d.get("cache_read_input_token_cost") or pin * 0.10 / 1e6) * 1e6,
            "cw5": (d.get("cache_creation_input_token_cost") or pin * 1.25 / 1e6) * 1e6,
            "cw1": (d.get("cache_creation_input_token_cost_above_1hr") or pin * 2.0 / 1e6) * 1e6,
        }
    if not models:
        raise RuntimeError("公開単価表に Anthropic モデルが見つかりません")
    return models


def _get_pricing():
    """公開単価 (日次キャッシュ)。失敗中は None (呼び出し側で内蔵表に切替)。"""
    today = datetime.date.today().isoformat()
    now = time.monotonic()
    with _pricing_lock:
        st = _pricing_state["data"]
        if st and st.get("date") == today:
            return st
        if now < _pricing_state["retry_at"]:
            return st  # 失敗直後: 前日値があればそれ、無ければ None

    # ファイルキャッシュ (成功時のみ書かれる)
    try:
        with open(PRICING_CACHE, encoding="utf-8") as f:
            cached = json.load(f)
        if cached.get("date") == today and cached.get("models"):
            with _pricing_lock:
                _pricing_state["data"] = cached
            return cached
    except Exception:
        cached = None

    try:
        models = _fetch_public_pricing()
        result = {"date": today, "models": models, "source": "litellm",
        ...
        try:
            ...
            pass
        with _pricing_lock:
            _pricing_state["data"] = result
        return result
    except Exception:
        with _pricing_lock:
            _pricing_state["retry_at"] = now + PRICING_RETRY
            if cached and cached.get("models"):
                # 前日のキャッシュでも内蔵表よりは新しい可能性が高いので使う
                _pricing_state["data"] = cached
                return cached
            return _pricing_state["data"]


def price_for(model: str):
    """モデルの単価詳細 {in,out,cr,cw5,cw1} (USD/1M) を返す。

    公開単価表に一致するモデルがあればその実値、無ければ内蔵表で概算。
    """
    m = (model or "").lower()
    ...
    if pricing and pricing.get("models"):
        ...
        if m in models:
            return models[m]
        # 日付サフィックス付き ID (claude-xxx-20260101) は素の ID に落として再検索
        base = re.sub(r"-\d{8}$", "", m)
        if base in models:
            return models[base]
    return _builtin_price(model)
...
def build_payload():
    ...
    return {
        ...
        "pricing": ({"source": pricing.get("source"), "date": pricing.get("date"),
                     "fetched_at": pricing.get("fetched_at"), "models": len(pricing.get("models", {}))}
                    if pricing else {"source": "builtin"}),
        "limits": limits,
        "claude": claude,
        "calendar": calendar,
...
function render(p){
  ...
  h += `<div class="cards">`
    ...
    + `</div>`;
  const pr = p.pricing || {};
  const prNote = pr.source === "litellm"
    ? `単価は公開単価表から毎日自動取得（${pr.date||""} 時点・${fmt(pr.models)}モデル）`
    : `単価は内蔵の概算表を使用中（公開単価表の取得に失敗・自動再試行します）`;
  h += `<div class="note">キャッシュ書込 5分TTL ${tokM(ct.cw5)} / 1時間TTL ${tokM(ct.cw1)} ／ 対象ファイル ${fmt(c.files)}件。コストは推定値 — ${prNote}（Web検索は $10/1000req 込み）。</div>`;
...
def _native_window(url, webview, on_gui_ready=None):
    ...
    デッドロック回避 (main() のコメント参照) のため:
    - webview モジュールは main() が import 済みのものを受け取る
    - バックグラウンドスレッド (HTTPサーバ on_gui_ready・アイコン・バー生成)
      は webview.start(func=...) で GUI/CLR 初期化完了後にまとめて開始する
    """
    try:
        _log("window: 本体ウィンドウ作成")
        ...
        use_overlay = not _flag("AI_USAGE_NO_OVERLAY")

        def _spawn_overlay():
            """本体の表示後にバーを作り、消えていたら作り直し続ける。"""
            try:
                main.events.shown.wait(30)  # 本体の表示完了を待つ
            except Exception:
                time.sleep(8)
            time.sleep(1.0)
            while not _windows.get("quitting"):
                try:
                    ...
                    if ov is None or ov not in webview.windows:
                        ov = _create_overlay(webview, url)
                        _windows["overlay"] = ov
                        _autofit_overlay(webview, ov)  # 内容幅に合わせて余白を除く
                except Exception:
                    pass
                time.sleep(5)
        ...
        def _on_ready():
            """GUI ループ開始後に呼ばれる。ここで初めて他スレッドを動かす。

            アイコン適用 (pythonnet) とバー生成を並走させると .NET 相互運用が
            デッドロックするため (実測 約50%)、必ずこの順で直列に行う。
            """
            _log("window: GUI 初期化完了 — バックグラウンド処理を開始")
            if on_gui_ready is not None:
                try:
                    on_gui_ready()   # HTTP サーバのリクエスト処理を開始
                except Exception:
                    pass
            if not _flag("AI_USAGE_DIAG_NOICON"):
                _apply_window_icon(main)      # 同期 (バー生成より先に完了させる)
            _log("window: アイコン適用完了")
            if use_overlay and not _flag("AI_USAGE_DIAG_NOBAR"):
                threading.Thread(target=_spawn_overlay, daemon=True).start()
            _log("window: on_ready 完了")

        _log("window: webview.start() 呼び出し")
        webview.start(func=_on_ready)  # 全ウィンドウが閉じられるまでブロック
        _log("window: webview.start() 復帰 (全ウィンドウ終了)")
        return True
    except Exception as e:
...
def main():
    ...
    if not _acquire_single_instance():
        ...
        return
    _log("main: 単一インスタンス確保")

    # 重要: pywebview (pythonnet 経由の .NET CLR 初期化) は、他のスレッドが
    # 動き出す前にシングルスレッドで済ませる。HTTP サーバのリクエスト処理と
    # CLR 初期化が重なると、まれにプロセス全体がデッドロックする (実測)。
    gui = None
    if not (_flag("AI_USAGE_NO_WINDOW") or _flag("AI_USAGE_NO_OPEN")):
        try:
            import webview as gui  # noqa: F401
            _log("main: pywebview 読み込み完了")
        except Exception as e:
            gui = None
    ...
    try:
        ...
        _focus_existing_window()
        return
    _log(f"main: サーバ bind 完了 ({BIND_HOST}:{PORT})")

    # 重要: リクエスト処理スレッドはまだ開始しない。webview.start() の
    # GUI/CLR 初期化と Python スレッドが並走すると、まれにプロセス全体が
    # デッドロックする (実測: 初期化中に固まり accept まで停止)。listen 済み
    # なので、開始前に来た接続は OS の backlog に溜まり失われない。
    serve_started = {"done": False}

    def _start_serving():
        if serve_started["done"]:
            return
        serve_started["done"] = True
        threading.Thread(target=server.serve_forever, daemon=True).start()
        _log("main: リクエスト処理を開始")
    ...
    try:
        ...
        elif gui is not None and _native_window(url, gui, _start_serving):
            pass  # ウィンドウが閉じられた
